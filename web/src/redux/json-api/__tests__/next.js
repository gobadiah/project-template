...
describe('HOC api', () => {
  ...
  it('should getMe when on server and if access_token cookie is present', async () => {
    ...
    const store = require('../../__fixtures__/store').default;
    const { user } = require('../../__fixtures__/store');
    const id = 1;
  ...
  it('should call currentUser if there is no access_token', () => {
    const { getMe, currentUser } = require('../..');
    const store = require('../../__fixtures__/store').default;
    const { user } = require('../../__fixtures__/store');
    jest.mock('../../selectors');
